...
export default function Index({ products, filters = {} }) {
    ...
    const applyFilter = (key, value) => {
        router.get('/shop-admin/products', { ...filters, [key]: value }, { preserveState: true, replace: true });
    };

    const removeProduct = () => {
        if (!deleteTarget) return;
        destroy(`/shop-admin/products/${deleteTarget.id}`, { onSuccess: () => setDeleteTarget(null) });
    };
        ...
        <AdminLayout title="Products">
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex gap-2">
                    <Link href="/shop-admin/products/create?type=medicine" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-2 font-semibold text-white shadow-lg shadow-emerald-600/20"><Icon name="plus" /> Medicine</Link>
                    <Link href="/shop-admin/products/create?type=glass" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-2 font-semibold text-white shadow-lg shadow-blue-600/20"><Icon name="plus" /> Glass</Link>
                </div>
            </div>
                ...
                {products.data.map(product => (
                    <article key={product.id} className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900 dark:ring-slate-800">
                        <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex gap-2">
                                <Link href={`/shop-admin/products/${product.id}/edit`} className="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"><Icon name="edit" /></Link>
